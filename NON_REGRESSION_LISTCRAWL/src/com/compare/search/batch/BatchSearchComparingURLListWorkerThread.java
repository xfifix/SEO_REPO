	...
	public BatchSearchComparingURLListWorkerThread(Connection con ,List<Integer> to_fetch, String my_user_agent) throws SQLException{
		...
		if (to_fetch.size()>0){
			try {
				...
				my_url="SELECT SEARCH_EXPRESSION, ID FROM SOLR_VS_EXALEAD_SEARCH_LIST WHERE TO_FETCH = TRUE and ID in "+to_fetch.toString();
...
//		}
	...
	private List<FacettesURLComparisonInfo> processComparison(List<ExpressionId> line_infos) {
		...
		for(ExpressionId line_info : line_infos){
			...
			// getting the URL
			String expression = line_info.getExpression();
			try{
				// fetching the solr version
				String solrurl = expression + "?b";
...
//				// get the cookies
				...
				String exaleadurl = expression + "?a";
				...
				System.out.println("Trouble fetching expression : "+expression);
				e.printStackTrace();
			}
