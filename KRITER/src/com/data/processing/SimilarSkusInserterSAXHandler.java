...
public class SimilarSkusInserterSAXHandler extends DefaultHandler {

	private Connection con;
	private static int batch_size = 10000;
	private static String insert_statement = "INSERT INTO SIMILAR_PRODUCTS(SKU,SKU1,SKU2,SKU3,SKU4,SKU5,SKU6) VALUES(?,?,?,?,?,?,?)";
	public SimilarSkusInserterSAXHandler(Connection con){
		super();
		this.con = con;
	}
	...
	public void insert_cache(){
		...
		try{
			...
			while (it.hasNext()){
				...
				st.setString(1,current_sku.getName());
				st.setString(2,similar_skus.get(0).getName());
				st.setString(3,similar_skus.get(1).getName());
				st.setString(4,similar_skus.get(2).getName());
				st.setString(5,similar_skus.get(3).getName());
				st.setString(6,similar_skus.get(4).getName());
				st.setString(7,similar_skus.get(5).getName());
				st.addBatch();
			}
			st.executeBatch();
			con.commit();
