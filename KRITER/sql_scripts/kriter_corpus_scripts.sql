CREATE TABLE IF NOT EXISTS KRITER_CORPUS_WORDS(
    WORD TEXT,
    NB_DOCUMENTS INT,
    DOC_LIST TEXT,
    LAST_UPDATE DATE
...
CREATE INDEX ON KRITER_CORPUS_WORDS(WORD);
...
select * from KRITER_CORPUS_WORDS where WORD='****'

if not found, we insert a row with the word, the url and the number of documents to one
# insert statement 
INSERT INTO KRITER_CORPUS_WORDS(WORD,NB_DOCUMENTS,DOC_LIST) values(?,?,?)
...
UPDATE KRITER_CORPUS_WORDS WORD=?,NB_DOCUMENTS=?,DOC_LIST=? WHERE WORD=?";
