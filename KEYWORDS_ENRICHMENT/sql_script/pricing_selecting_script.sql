...
## creating high trafic ranking keywords
 select cd.keyword, cd.search_volume, cd.search_position from CDISCOUNT_PRICING as cd where cd.keyword in (select keyword from HIGH_RANKING_KEYWORDS) order by cd.search_volume desc


## we don't rank on those keywords
 select cd.keyword, cd.url, cd.search_volume, cd.search_position INTO CDISCOUNT_WITH_HIGH_TRAFIC from CDISCOUNT_PRICING as cd where cd.keyword in (select keyword from HIGH_RANKING_KEYWORDS) order by cd.search_volume desc

 select cd.keyword, cd.url, cd.search_volume, cd.search_position INTO CDISCOUNT_WITH_NOT_HIGH_TRAFIC from CDISCOUNT_PRICING as cd where cd.keyword not in (select keyword from HIGH_RANKING_KEYWORDS) order by cd.search_volume desc
