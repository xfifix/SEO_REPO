...
import com.offer.utility.OfferMatchingUtility;

public class ExaleadOfferMatchingPostExample {


	public static void main(String[] args) throws ClientProtocolException, IOException{
		// semantic input
		String title = "Moule à bûche de Noël antiadhésif 33 x 15 x 6.5 cm noir";
		String h1 = "Moule à gâteau Moule à bûche de Noël antiadhésif 33 x 15 x 6.5 cm noir Référence : FFO5097271";
		
		String semantic_hits = "cuisine jardin marques accueil plat linge table décoration meuble enfant";
		String semantic_title = "noir moule cm noël bûche 6.5 antiadhésif 15 33";
		String inlinks_semantic = "[Revenir en haut de page, Moule à bûche de Noël antiadhésif 33 x 15 x 6.5 cm noir, Facilités de paiement]";
		String inlinks_semantic_count = "{Revenir en haut de page=1, Moule à bûche de Noël antiadhésif 33 x 15 x 6.5 cm noir=1, Facilités de paiement=1}";
		...
		// we loop over each word, we request exalead and we see if there is offer matching
		// we count the number of occurences where offer was found
		// we affect a percentage value to the page which represents the Cdiscount offer matching paradigm 
		for (String semanticQuery : myQueriesSet){
			System.out.println("Querying for semantic keyword : "+semanticQuery);
			List<ULRLineToInsert> results = fetch_results(semanticQuery);
			System.out.println("Number of results found : "+results.size()+" for "+semanticQuery);
		}
	}

	private static List<ULRLineToInsert> fetch_results(String semanticQuery) throws ClientProtocolException, IOException{
		String number = "10";
		List<ULRLineToInsert> toresult = new ArrayList<ULRLineToInsert>();
		OfferMatchingRequestResults result = new OfferMatchingRequestResults();
		int volume=10;
		result.setRequest(semanticQuery);
		...
		nvps.add(new BasicNameValuePair("q", semanticQuery));
		...
		try {
			...
			// do something useful with the response body
			// and ensure it is fully consumed
			toresult = parse_results(utf_8_value,result.getRequest());
